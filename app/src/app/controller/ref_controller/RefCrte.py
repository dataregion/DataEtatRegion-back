...
api = Namespace(name="CRTE Controller", path="/crte", description="API referentiels des CRTE")
...
crte_model = api.model("crte", model={"code": fields.String(), "nom": fields.String()})
...
class RefCrte(Resource):
    ...
    def get(self):
        ...
        if name is None and dept is None:
            sql_execute = text(f"{sql_select} ORDER BY label_crte LIMIT :limit").bindparams(
            ...
            sql_execute = text(
                f"{sql_select} WHERE {' AND '.join(where_clause)} ORDER BY label_crte LIMIT :limit"
            ).bindparams(*params)
