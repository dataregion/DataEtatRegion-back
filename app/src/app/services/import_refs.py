...
LIST_REF_TASK_IMPORT: list[RefTaskImportChorus] = [
    RefTaskImportChorus(
        cls_name="CodeProgramme",
        columns=["code_ministere", "code", "label"],
        usecols=[0, 2, 3],
    ...
    RefTaskImportChorus(
        cls_name="Ministere",
        columns=["code", "sigle_ministere", "label"],
        usecols=[0, 1, 2],
        sheet_name="02 - Ministères",
        skiprows=8,
    ),
    RefTaskImportChorus(
        cls_name="DomaineFonctionnel",
        columns=["code", "label"],
        usecols=[4, 5],
...
def import_refs(file, data, username=""):
    ...
    if file and allowed_file(file.filename, {"csv", "ods", "xls", "xlsx"}):
        ...
        try:
            db.session.add(AuditUpdateData(username=username, filename=file.filename, data_type=DataType.REFERENTIEL))
            db.session.commit()
        except Exception:
...
def import_ref_calculette(file, username=""):
    ...
    if file and allowed_file(file.filename, allowed_extensions={"xlsx"}):
        ...
        for ref in LIST_REF_TASK_IMPORT:
            logging.info(f"[IMPORT REF] Send task Maj ref for {ref.cls_name}")
            import_refs_task.delay(
                str(save_path),
                ref.cls_name,
                ref.columns,
                is_csv=False,
                usecols=ref.usecols,
                skiprows=ref.skiprows,
