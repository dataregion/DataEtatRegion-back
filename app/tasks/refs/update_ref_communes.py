...
def import_file_acv_from_website(self, url: str):
    ...
    with tempfile.NamedTemporaryFile(delete=False) as temp_file:
        ...
        _logger.info("Fichier téléchargé. Import...")
        _send_subtask_acv_file(temp_file.name)


@limiter_queue(queue_name="file")
def _send_subtask_acv_file(filepath: str):
    subtask("import_file_acv").delay(filepath)


@_celery.task(bind=True, name="import_file_acv")
def import_file_acv(self, fichier: str):
    # get file
    _logger.info(f"[IMPORT][ACV] Start for file {fichier}")
    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")

    move_folder = current_app.config["UPLOAD_FOLDER"] + "/save/"

    try:
        data_acv_chunk = pd.read_csv(
            fichier,
            sep=",",
            header=0,
            dtype={"insee_com": str, "lib_com": str, "date_signature": str},
        ...
        for chunk in data_acv_chunk:
            updates_to_commit = []

            # Récupération des dates de signature pour chaque commune
            dates_by_code = dict()
            for _, acv_data in chunk.iterrows():
                commune_json = json.loads(acv_data.to_json())
                code_commune = commune_json.get("insee_com", None)
                date_signature = commune_json.get("date_signature", None)
                dates_by_code[code_commune] = date_signature
            ...
            # Bulk update
            set_communes_acv(updates_to_commit)

        move_folder = os.path.join(move_folder, timestamp)
        if not os.path.exists(move_folder):
            os.makedirs(move_folder)
        logger.info(f"[IMPORT][ACV] Save file {fichier} in {move_folder}")

        shutil.move(fichier, move_folder)
        logger.info("[IMPORT][ACV] End")

        return True
    except Exception as e:
        logger.exception(f"[IMPORT][ACV] Error lors de l'import du fichier ACV: {fichier}")
        raise e
