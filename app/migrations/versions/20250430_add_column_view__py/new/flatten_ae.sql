CREATE OR REPLACE VIEW public.flatten_ae AS
  select DISTINCT ON (root.n_ej, root.n_poste_ej, root.source_region)
    'FINANCIAL_DATA_AE'::text AS source,
    root.id,
    root.n_poste_ej,
    root.n_ej,
    root.annee,
   ...
   FROM financial_ae root
     ...
     LEFT JOIN (  SELECT id_financial_ae, SUM(montant) AS montant_ae
	    ...
	    GROUP BY id_financial_ae
		 ) aggr_mt_ae ON aggr_mt_ae.id_financial_ae = root.id
     ...
     ORDER BY root.n_ej, root.n_poste_ej, root.source_region, case when root.data_source = 'NATION' then 1 else 2 end;