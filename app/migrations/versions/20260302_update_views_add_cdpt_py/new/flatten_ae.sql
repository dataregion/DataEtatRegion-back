-- public.flatten_ae source

CREATE OR REPLACE VIEW public.flatten_ae
AS SELECT DISTINCT ON (root.n_ej, root.n_poste_ej, root.source_region) 'FINANCIAL_DATA_AE'::text AS source,
    root.id,
    root.n_poste_ej,
    root.n_ej,
    root.annee,
    ...
    rcc.code_departement AS "centreCouts_codeDepartement"
   FROM financial_ae root
     ...
     LEFT JOIN ref_qpv rs_rqpv15 ON rs.code_qpv15::text = rs_rqpv15.code::text
     LEFT JOIN ref_qpv rs_rqpv24 ON rs.code_qpv24::text = rs_rqpv24.code::text
     ...
     LEFT JOIN qpv_lieu_action qla ON root.n_ej::text = qla.ej::text
     LEFT JOIN ref_qpv refq ON qla.code_qpv::text = refq.code::text
     LEFT JOIN ( SELECT montant_financial_ae.id_financial_ae,
            sum(montant_financial_ae.montant) AS montant_ae
           FROM montant_financial_ae
          GROUP BY montant_financial_ae.id_financial_ae) aggr_mt_ae ON aggr_mt_ae.id_financial_ae = root.id
  ...
  ORDER BY root.n_ej, root.n_poste_ej, root.source_region, (
        CASE
            WHEN root.data_source::text = 'NATION'::text THEN 1
            ELSE 2
        END);