...
class Donnee(db.Model):
    ...
    demarche_number: Column[int] = Column(Integer, ForeignKey("demarche.number"), nullable=False)
    demarche: Mapped[Demarche] = relationship("Demarche", lazy="select")
    section_name: Column[int] = Column(Integer, ForeignKey("section.name"), nullable=False)
    section: Mapped[Section] = relationship("Section", lazy="select")
    type_name: Column[int] = Column(Integer, ForeignKey("type.name"), nullable=False)
