...
def upgrade():
    ...
    df_programme = pandas.read_csv(
        code_programme_file,
        sep=",",
        usecols=["code_programme", "titre_programme", "theme"],
        dtype={"code_programme": str, "titre_programme": str, "theme": str},
    )
    try:
        for index, programme in df_programme.iterrows():
            instance_programme = (
                session.query(_CodeProgramme).filter_by(**{"code": programme["code_programme"]}).one_or_none()
            )
            ...
            if instance_programme is None:
                programme = _CodeProgramme(
                    code=programme["code_programme"],
                    label=programme["titre_programme"],
