...
def run_performance_test(users=10, spawn_rate=2, run_time="60s"):
    ...
    try:
        ...
        print(
            "❌ Erreur: Locust n'est pas installé. Installez-le avec: pip install locust"
        )
        return False
...
def main():
    ...
    parser.add_argument(
        "--users", type=int, default=10, help="Nombre d'utilisateurs simultanés"
    )
    parser.add_argument(
        "--spawn-rate", type=int, default=2, help="Taux de montée (utilisateurs/sec)"
    )
    ...
    success = run_performance_test(
        users=args.users, spawn_rate=args.spawn_rate, run_time=args.run_time
    )
