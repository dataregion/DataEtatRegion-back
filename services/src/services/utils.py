...
def convert_exception(exception_to_transform, exception_to_raise):
    ...
    if not issubclass(exception_to_raise, BaseException) or not issubclass(exception_to_transform, BaseException):
        raise ValueError("Mauvaise utilisation du décorateur. ( e.g: @convert_exception(Err, Err) )")
