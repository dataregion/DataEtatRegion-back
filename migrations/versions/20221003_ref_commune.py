...
class _Commune(_Base):
    ...
    code_commune: Column[str] = Column(String, unique=True, nullable=False)
...
def upgrade():
    ...
    op.execute(
        "INSERT INTO ref_commune (code_commune)"
        "SELECT DISTINCT code_commune FROM ref_siret WHERE code_commune NOT IN ( SELECT code_commune FROM ref_commune) "
    )

    op.alter_column("ref_siret", "code_commune", existing_type=sa.VARCHAR(), nullable=True)
...
def _insert_ref():
    ...
    try:
        for index, programme in data_frame.iterrows():
            ...
            if commune is None:
                commune = _Commune(
                    code_commune=programme["insee_com"],
                    code_crte=programme["id_crte"],
                    label_crte=programme["lib_crte"],
                    label_commune=programme["lib_com"],
                )
                session.add(commune)
