...
def upgrade():
    ...
    sql_insert_montant = (
        "INSERT INTO montant_financial_ae (id_financial_ae, montant, annee, created_at, updated_at) "
    ...
    view_montant = (
        "CREATE OR REPLACE VIEW public.montant_par_commune AS "
        "SELECT SUM(dc.montant) as montant, rs.code_commune FROM ("
        + sql_tmp_sum_montant
        + ") AS dc LEFT JOIN ref_siret AS  rs ON (rs.code = dc.siret) GROUP BY rs.code_commune"
    )

    view_theme = (
        "CREATE OR REPLACE VIEW public.montant_par_commune_type_theme AS "
        "SELECT SUM(dc.montant) AS montant, rs.code_commune, rcj.type AS type, rt.label as theme FROM ("
        + sql_tmp_sum_montant
        + ") as dc LEFT JOIN ref_siret AS  rs ON (rs.code = dc.siret) LEFT JOIN ref_code_programme AS rcp ON (rcp.code = dc.programme) LEFT JOIN ref_theme AS rt ON (rt.id = rcp.theme) LEFT JOIN ref_categorie_juridique AS rcj ON (rcj.code = rs.categorie_juridique) GROUP BY (rs.code_commune,rcj.type, rt.label)"
    )

    view_type = (
        "CREATE OR REPLACE VIEW public.montant_par_commune_type AS "
        "SELECT SUM(dc.montant) AS montant, rs.code_commune, rcj.type as type FROM ("
        + sql_tmp_sum_montant
        + ") as dc LEFT JOIN ref_siret AS  rs ON (rs.code = dc.siret) LEFT JOIN ref_categorie_juridique AS rcj ON (rcj.code = rs.categorie_juridique) GROUP BY (rs.code_commune,rcj.type)"
    )
    ...
    view_programme_annee_type = (
        "CREATE OR REPLACE VIEW  public.montant_par_niveau_bop_annee_type AS ("
        "SELECT SUM(dc.montant) AS montant, 'commune' as niveau, rs.code_commune as code, dc.programme, dc.annee, rcj.type as TYPE 	FROM ("
        + sql_tmp_sum_montant_group_annee
        + ") as dc "
        ...
        "UNION ("
        "SELECT SUM(dc.montant) AS montant, 'epci' as niveau, rc.code_epci as code, dc.programme, dc.annee, rcj.type as TYPE FROM ("
        + sql_tmp_sum_montant_group_annee
        + ") as dc "
        ...
        "UNION ("
        "SELECT SUM(dc.montant) AS montant, 'departement' as niveau, rc.code_departement as code, dc.programme, dc.annee, rcj.type as type FROM ("
        + sql_tmp_sum_montant_group_annee
        + ") as dc "
        "LEFT JOIN ref_siret AS  rs ON (rs.code = dc.siret) "
        "LEFT JOIN ref_commune AS  rc ON (rc.code = rs.code_commune) "
