...
def _grouping_mechanisme(grouping: list[Colonne], grouped: list[str] | None, conditions: dict):
    print(grouping)
    print(grouped)
    if grouped is None or len(grouped) == 0:
        return grouping[0], conditions
    conditions[grouping[0].code] = { "value": grouped[0] }
    if grouping[0].type is not None:
        conditions[grouping[0].code]['type'] = grouping[0].type
    return _grouping_mechanisme(grouping[1:], grouped[1:], conditions)

class BuilderQueryFinancialLine:
    """
    ...
    def __init__(self, session: Session, params: BudgetQueryParams) -> None:
        ...
        self.message = "Liste des données financières"
        ...
        if params.grouping is not None and (params.grouped is None or len(params.grouped) < len(params.grouping)):
            self.groupby_colonne, self.dynamic_conditions = _grouping_mechanisme(params.grouping, params.grouped, {})
            colonnes = [
            ...
            self._stmt = select(*colonnes)
            self.message = "Liste des montants agrégés"
        else:
            # No grouping: use colonnes param
            if params.colonnes is not None:
                selected_fields = [getattr(FinancialLines, c) for c in params.colonnes if hasattr(FinancialLines.__dict__, c)]
                self._stmt = select(*selected_fields)
            else:
                self._stmt = select(FinancialLines)
    ...
    def add_dynamic_condition(self, colonne: str, value: dict):
        print("rrrrrrrrrrrrr")
        print(value)
        value = value['type'](value['value']) if 'type' in value else value['value']
        print(value)
        self._stmt = self._stmt.where(getattr(FinancialLines, colonne) == value)
