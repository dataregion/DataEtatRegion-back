...
class V3QueryBuilder(Generic[T]):
    ...
    def get_total(self, type: str):
        if type != "groupings":
            ...
            model = self._query.order_by(None).limit(None).offset(None).subquery()
            q = select(
                func.coalesce(func.sum(model.c.total), 0).label("total"),
                func.coalesce(func.sum(model.c.total_montant_engage), 0).label("total_montant_engage"),
                func.coalesce(func.sum(model.c.total_montant_paye), 0).label("total_montant_paye"),
            )
